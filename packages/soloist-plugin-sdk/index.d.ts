/** Public authoring contract for manifests with `soloistApiVersion: "1"`. */
export type SoloistApiVersion = "1";
export type ScriptPluginPermission =
  | "storage"
  | "slashCommands:register"
  | "oracleProviders:register"
  | "document:readSelection"
  | "document:insertBlock";
export type ScriptPluginManifest = {
  id: string;
  name: string;
  version: string;
  soloistApiVersion: SoloistApiVersion;
  type: "script";
  entry: string;
...
export type PluginLocalStorage = {
  get<T extends PluginJsonValue = PluginJsonValue>(key: string): Promise<T | undefined>;
  set<T extends PluginJsonValue = PluginJsonValue>(key: string, value: T): Promise<void>;
...
export type PluginStatus = { id: string; level: PluginStatusLevel; message: string };
...
export type PluginResultBlockType =
  | "roll" | "oracle" | "scene" | "combat" | "stat" | "chaos" | "error";
...
export type PluginCommandExecutionResult =
  | { type: "insertResultBlock"; block: PluginResultBlock; display?: "inline" | "block" }
  | { type: "deleteCommand" };
...
export type OracleOdds =
  | "impossible" | "no_way" | "very_unlikely" | "unlikely" | "50_50"
  | "likely" | "very_likely" | "near_sure" | "sure_thing";
...
export type SceneSetupInput = { prompt: string; roll: number; chaosFactor: number };
