...
import type { CommandExecutionResult } from "../commands/executeCommand";
...
function replacementNodesForBlock(input: ApplyCommandResultInput, node: ProseMirrorNode) {
  ...
  const afterParagraph =
    paragraphWithContent(paragraph, cursor.parentOffset, paragraph.content.size) ??
...
function replaceCommandParagraph(input: ApplyCommandResultInput, node: ProseMirrorNode) {
...
export function applyCommandResult(input: ApplyCommandResultInput): boolean {
  ...
  if (result.type === "insertCombatSpace") {
    ...
    return combatSpaceNode ? replaceCommandParagraph(input, combatSpaceNode) : false;
  }
  ...
  const spacerNode = shouldInsertSpacer ? editor.schema.nodes.paragraph.create() : null;
  const insertPos =
  ...
  const oldTurnNode = combatTurnDepth === null ? null : cursor.node(combatTurnDepth);
  const oldTurnPos =
